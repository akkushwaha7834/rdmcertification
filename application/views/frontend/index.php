<!-- Banner section -->
<section class="bg-image zoomoutheader overflow-hidden position-relative bg-overlay-black-50 z-1"
    style="background-image:url('<?=base_url()?>public/web/assets/images/banner.webp')">
    <div class="container position-relative z-1">
        <div class="row justify-content-center">
            <div class="col-md-10">
                <div class="banner-caption text-center">
                    <h1 id="banner-head" class=""></h1>
                    <p class="wow fadeInUp mb-5" data-wow-delay="0.4s">Lorem ipsum dolor sit amet consectetur adipisicing elit. Natus nihil architecto quia sit neque veniam illum, voluptates beatae amet doloribus.</p>
                    <a href="" class="default-btn wow fadeInUp" data-wow-delay="0.5s">Read More <i
                            class="ri-arrow-right-line"></i></a>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="">
    <div class="bg-gradient pt-3 pb-2">
        <marquee behavior="" direction="" onmouseover="stop()" onmouseout="start()">
            <div class="d-flex gap-3 align-items-center">
                <div>
                    <p class="mb-0 fw-semibold text-white">Research & Development Management Certification Services
                        provide all
                        certificate – </p>
                </div>
                <div class="marquee-box">ISO 9001</div>
                <div class="text-white">•</div>
                <div class="marquee-box">ISO 10002</div>
                <div class="text-white">•</div>
                <div class="marquee-box">ISO 13485</div>
                <div class="text-white">•</div>
                <div class="marquee-box">ISO 14001</div>
                <div class="text-white">•</div>
                <div class="marquee-box">ISO 18001</div>
                <div class="text-white">•</div>
                <div class="marquee-box">ISO 20000</div>
                <div class="text-white">•</div>
                <div class="marquee-box">ISO 22000</div>
                <div class="text-white">•</div>
                <div class="marquee-box">ISO 27001</div>
                <div class="text-white">•</div>
                <div class="marquee-box">ISO 29990</div>
                <div class="text-white">•</div>
                <div class="marquee-box">ISO 31000</div>
                <div class="text-white">•</div>
                <div class="marquee-box">SA 8000</div>
                <div class="text-white">•</div>
                <div class="marquee-box">ISO/TS 16949</div>
                <div class="text-white">•</div>
                <div class="marquee-box"> HACCP</div>
                <div class="text-white">•</div>
                <div class="marquee-box">GMP</div>
                <div class="text-white">•</div>
                <div class="marquee-box">CE</div>
                <div class="text-white">•</div>
                <div class="marquee-box">KOSHER< HALAL in best price</div>
                </div>

        </marquee>
    </div>
</section>

<section class="section-space bg-image2" style="background-image:url('<?=base_url()?>public/web/assets/images/bgg.png')">
    <div class="container position-relative z-1">
        <div class="row g-4 section-space pt-0">
            <div class="col-lg-6 text-center"><img src="<?=base_url()?>public/web/assets/images/aboutus.png" alt="" class="w-75"></div>
            <div class="col-lg-6">
                <h3>About Us</h3>
                <h2 class="mb-4 h1">Lorem ipsum dolor sit amet</h2>
                <p>Research & Development Management Certification Services gives value added assessment by its trained, quality conscious, experienced auditors & technical experts. RDM Certification Services provides no. of certifications that ensure your quality and standard. We also help to explore Quality Management System’s benefits in a friendly manner so that you can optimize your organisation’s performance.  unbiased auditing service with wide range of scopes to its clients.</p>
                <a href="about.php" class="default-btn mt-4">More About Us <i class="ri-arrow-right-line"></i></a>
            </div>
        </div>
        <div class="row g-4">
            <div class="col-lg-6 col-md-6">
                <div class="boxes-wrap">
                    <img src="<?=base_url()?>public/web/assets/images/promotes2.png" alt="" class="mb-4">
                    <h3 class="">Promotes best practices</h3>
                    <p>IT services refers to the application of business and technical expertise to enable organizations   in the creation, management and optimization of or access to information and business processes.
                    </p>
                </div>
            </div>
            <div class="col-lg-6 col-md-6">
                <div class="boxes-wrap">
                    <img src="<?=base_url()?>public/web/assets/images/productive.png" alt="" class="mb-4">
                    <h3 class="">Helps you become more productive</h3>
                    <p>Adhering to standards requires you to clearly define, document and monitor your business processes. You also need to set objectives for your company and measure your progress. This work  is critical to building a lean, productive business.</p>
                </div>
            </div>
            <div class="col-lg-6 col-md-6">
                <div class="boxes-wrap">
                    <img src="<?=base_url()?>public/web/assets/images/punctuality.png" alt="" class="mb-4">
                    <h3 class="">Punctuality</h3>
                    <p>The importance of being on time is our first priority. It should be no surprise, we complete all  the task with relevant way and on time.</p>
                </div>
            </div>
            <div class="col-lg-6 col-md-6">
                <div class="boxes-wrap">
                    <img src="<?=base_url()?>public/web/assets/images/customers.png" alt="" class="mb-4">
                    <h3 class="">Keeps customers happy</h3>
                    <p>Standards help keep customers satisfied by improving complaint management, quality control and client satisfaction monitoring. Research indicates one of the top benefits from certification is  reduced customer complaints..</p>
                </div>
            </div>

        </div>
    </div>
</section>

<section class="bg-dark section-space">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-10 text-center">
                <h3 class="text-white">Our Services</h3>
                <h2 class="mb-4 text-white h1 mb-5">Lorem ipsum dolor sit amet</h2>
            </div>
        </div>
        <div class="row justify-content-center">
            <div class="col-md-10">
                <div class="swiper service-slider px-1 py-3">
                    <div class="swiper-wrapper pb-5">
                        <div class="swiper-slide">
                            <div class="service-wrap">
                                <div class="row">
                                    <div class="col-lg-6 align-self-center">
                                        <div class="service-wrap-item">
                                            <h3 class="text-white mb-4 fw-semibold">ISO Certification</h3>
                                            <p class="text-white ">RDM Certification Services provides no. of  certifications that ensure your quality and standard. Our range of services has also grown and we now offer assessment of management,  Environment, and Health safety management systems in most industry  sectors.</p>
                                            <a href="" class="btn-round mt-4"> Read More <i
                                                    class="ri-arrow-right-line"></i> </a>
                                        </div>
                                    </div>
                                    <div class="col-lg-6 px-0">
                                        <img src="<?=base_url()?>public/web/assets/images/iso-service.jpg" alt="" class="w-100 h-100">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="service-wrap">
                                <div class="row">
                                    <div class="col-lg-6 align-self-center">
                                        <div class="service-wrap-item">
                                            <h3 class="text-white mb-4 fw-semibold">IT Services</h3>
                                            <p class="text-white">RDM Certification Services provides no. of  certifications that ensure your quality and standard. Our range of   services has also grown and we now offer assessment of management, Environment, and Health safety management systems in most industry sectors.</p>
                                            <a href="" class="btn-round "> Read More <i class="ri-arrow-right-line"></i>
                                            </a>
                                        </div>
                                    </div>
                                    <div class="col-lg-6 px-0">
                                        <img src="<?=base_url()?>public/web/assets/images/it-services.webp" alt="" class="w-100 h-100">
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="swiper-pagination"></div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="counter-area bg-image section-space" style="background-image:url('<?=base_url()?>public/web/assets/images/our-case-bg.jpg')">
    <div class="container py-5">

        <div class="row g-4 text-center">
            <div class="col-lg-3 col-sm-6 ">
                <div class="single-counter">
                    <div class="count-title">
                        <h2 class="text-white">
                            <span class="odometer odometer-auto-theme" data-count="1200">
                                <div class="odometer-inside">
                                    <span class="odometer-digit">
                                        <span class="odometer-digit-spacer">8</span>
                                        <span class="odometer-digit-inner">
                                            <span class="odometer-ribbon">
                                                <span class="odometer-ribbon-inner">
                                                    <span class="odometer-value">1</span>
                                                </span>
                                            </span>
                                        </span>
                                    </span>
                                    <span class="odometer-digit">
                                        <span class="odometer-digit-spacer">8</span>
                                        <span class="odometer-digit-inner">
                                            <span class="odometer-ribbon">
                                                <span class="odometer-ribbon-inner">
                                                    <span class="odometer-value">2</span>
                                                </span>
                                            </span>
                                        </span>
                                    </span>
                                    <span class="odometer-digit">
                                        <span class="odometer-digit-spacer">8</span>
                                        <span class="odometer-digit-inner">
                                            <span class="odometer-ribbon">
                                                <span class="odometer-ribbon-inner">
                                                    <span class="odometer-value">0</span>
                                                </span>
                                            </span>
                                        </span>
                                    </span>
                                    <span class="odometer-digit">
                                        <span class="odometer-digit-spacer">8</span>
                                        <span class="odometer-digit-inner">
                                            <span class="odometer-ribbon">
                                                <span class="odometer-ribbon-inner">
                                                    <span class="odometer-value">0</span>
                                                </span>
                                            </span>
                                        </span>
                                    </span>
                                </div>
                            </span>
                            <span class="target">+</span>
                        </h2>
                        <p class="mb-0 text-white">Certificates</p>
                    </div>
                </div>
            </div>

            <div class="col-lg-3 col-sm-6 ">
                <div class="single-counter">
                    <div class="count-title">
                        <h2 class="text-white">
                            <span class="odometer odometer-auto-theme" data-count="200">
                                <div class="odometer-inside">
                                    <span class="odometer-digit">
                                        <span class="odometer-digit-spacer">8</span>
                                        <span class="odometer-digit-inner">
                                            <span class="odometer-ribbon">
                                                <span class="odometer-ribbon-inner">
                                                    <span class="odometer-value">2</span>
                                                </span>
                                            </span>
                                        </span>
                                    </span>
                                    <span class="odometer-digit">
                                        <span class="odometer-digit-spacer">8</span>
                                        <span class="odometer-digit-inner">
                                            <span class="odometer-ribbon">
                                                <span class="odometer-ribbon-inner">
                                                    <span class="odometer-value">0</span>
                                                </span>
                                            </span>
                                        </span>
                                    </span>
                                    <span class="odometer-digit">
                                        <span class="odometer-digit-spacer">8</span>
                                        <span class="odometer-digit-inner">
                                            <span class="odometer-ribbon">
                                                <span class="odometer-ribbon-inner">
                                                    <span class="odometer-value">0</span>
                                                </span>
                                            </span>
                                        </span>
                                    </span>

                                </div>
                            </span>
                            <span class="target text-white">+</span>
                        </h2>
                        <p class="mb-0 text-white">Satisfied Clients</p>
                    </div>
                </div>
            </div>

            <div class="col-lg-3 col-sm-6 ">
                <div class="single-counter">
                    <div class="count-title">
                        <h2 class="text-white">
                            <span class="odometer odometer-auto-theme" data-count="50">
                                <div class="odometer-inside">
                                    <span class="odometer-digit">
                                        <span class="odometer-digit-spacer">8</span>
                                        <span class="odometer-digit-inner">
                                            <span class="odometer-ribbon">
                                                <span class="odometer-ribbon-inner">
                                                    <span class="odometer-value">5</span>
                                                </span>
                                            </span>
                                        </span>
                                    </span>
                                    <span class="odometer-digit">
                                        <span class="odometer-digit-spacer">8</span>
                                        <span class="odometer-digit-inner">
                                            <span class="odometer-ribbon">
                                                <span class="odometer-ribbon-inner">
                                                    <span class="odometer-value">0</span>
                                                </span>
                                            </span>
                                        </span>
                                    </span>

                                </div>
                            </span>
                            <span class="target">+</span>
                        </h2>
                        <p class="mb-0 text-white">Products & Service</p>
                    </div>
                </div>
            </div>

            <div class="col-lg-3 col-sm-6 ">
                <div class="single-counter">
                    <div class="count-title">
                        <h2 class="text-white">
                            <span class="odometer odometer-auto-theme" data-count="450">
                                <div class="odometer-inside">
                                    <span class="odometer-digit">
                                        <span class="odometer-digit-spacer">8</span>
                                        <span class="odometer-digit-inner">
                                            <span class="odometer-ribbon">
                                                <span class="odometer-ribbon-inner">
                                                    <span class="odometer-value">4</span>
                                                </span>
                                            </span>
                                        </span>
                                    </span>
                                    <span class="odometer-digit">
                                        <span class="odometer-digit-spacer">8</span>
                                        <span class="odometer-digit-inner">
                                            <span class="odometer-ribbon">
                                                <span class="odometer-ribbon-inner">
                                                    <span class="odometer-value">5</span>
                                                </span>
                                            </span>
                                        </span>
                                    </span>
                                    <span class="odometer-digit">
                                        <span class="odometer-digit-spacer">8</span>
                                        <span class="odometer-digit-inner">
                                            <span class="odometer-ribbon">
                                                <span class="odometer-ribbon-inner">
                                                    <span class="odometer-value">0</span>
                                                </span>
                                            </span>
                                        </span>
                                    </span>
                                </div>
                            </span>
                            <span class="target">+</span>
                        </h2>
                        <p class="mb-0 text-white">Cup Of Coffees</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="clients-sec section-space">
    <div class="container">
        <div class="row justify-content-center mb-5">
            <div class="col-lg-10 text-center">
                <h3 class="text-primary">Our Clients</h3>
                <h2 class="mb-4 text-dark h1 mb-4">Lorem ipsum dolor sit amet</h2>
            </div>
        </div>
    </div>
    <div class="swiper client-slider py-3">
        <div class="swiper-wrapper align-items-center gap-4 ">
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c11.png" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c12.png" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c13.png" alt="">
                    </figure>
                </div>
            </div>

            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c15.png" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c16.png" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c17.png" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c18.png" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/logo (2).png" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/HTHllabs.png" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c7.jpg" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c6.jpg" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c4.jpg" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c3_.jpg" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c2_.jpg" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/c1.jpg" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/blueboyd.jpg" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/Bantia-logo_White_1_215x@2x.jpg" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/552.jpg" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/33.jpg" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide">
                <div>
                    <figure>
                        <img src="<?=base_url()?>public/web/assets/images/clients-logo/22.jpg" alt="">
                    </figure>
                </div>
            </div>
            <div class="swiper-slide"></div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/11.jpg" alt="">
                </figure>
            </div>
        </div>
    </div>
    </div>
    <!-- <marquee behavior="" direction="right" onmouseover="stop()" onmouseout="start()" >
        <div class="d-flex gap-5 align-items-center">
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c11.png" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c12.png" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c13.png" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/logo_2_0_0_1.png" alt="" width="77px" >
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c15.png" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c16.png" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c17.png" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c18.png" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c10.png" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c9.png" alt="">
                </figure>
            </div>
       
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c7.jpg" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c6.jpg" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c5.jpg" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c4.jpg" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c3_.jpg" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c2_.jpg" alt="">
                </figure>
            </div>

            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/c1.jpg" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/blueboyd.jpg" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/Bantia-logo_White_1_215x@2x.jpg" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/552.jpg" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/33.jpg" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/22.jpg" alt="">
                </figure>
            </div>
            <div>
                <figure>
                    <img src="<?=base_url()?>public/web/assets/images/clients-logo/11.jpg" alt="">
                </figure>
            </div>

    </marquee> -->

</section>

<section class="section-space form-area bg-image  position-relative" style="background-image:url('<?=base_url()?>public/web/assets/images/bg-shape.png');">
    <div class="tp-faq-bg"></div>
    <div class="container z-1 position-relative">
        <div class="row">
            <div class="col-lg-5">
                <h3 class="text-white">Ask Anything</h3>
                <h2 class="mb-4 h1 text-white">You can contact
                us for any question</h2>
                <p class="text-white mb-4">Lorem ipsum dolor sit amet consectetur adipisicing elit. Placeat deserunt ducimus iusto facilis, dolore.</p>
            </div>
            <div class="col-lg-7">
                <div class="form-wrapper">
                    <form action="">
                        <div class="row g-4">
                            <div class="col-md-6">
                                <input type="text" placeholder="First Name"  class="form-control">
                            </div>
                            <div class="col-md-6">
                                <input type="text" placeholder="Last Name" class="form-control" >
                            </div>
                            <div class="col-md-12">
                                <input type="text" placeholder="Company Name"  class="form-control">
                            </div>
                            <div class="col-md-12">
                                <input type="email" placeholder="Busines Email"  class="form-control">
                            </div>
                            <div class="col-md-12">
                                <input type="tel" placeholder="Contact Number"  class="form-control">
                            </div>
                            <div class="col-md-12">
                            <button class="default-btn border-0 ">Submit</button>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
    <div class="height-section">
    </div>
</section>